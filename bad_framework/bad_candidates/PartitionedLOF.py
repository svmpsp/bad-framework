class PartitionedLOF:
    """Implements a partitioned variant of the Local Outlier
    Factor (LOF) outlier detection method proposed in
    ...
    corresponding its LOF. LOF is computed as a function of the
    relative k-nearest-neighborhood density of each data element.

    In this variant, the LOF is computed with respect to a
    partition of the whole data set, where the partition size is
    defined as a fraction of the complete data set size.

    Parameters:
        - k: (int) number of neighbors to consider (defaults to 10). Must
        be smaller than the data set size.
        - partition_size: (int) integer number between 0 and 100. A partition_size of 100
        corresponds to original LOF.

    This class wraps the LOF implementation provided by the scikit-learn library
